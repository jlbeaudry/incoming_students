...
wordcount: X
bibliography: ['r-references.bib','Matt_Zotero_Library.bib']
floatsintext: yes
figurelist: no
tablelist: no
footnotelist: no
...
<!-- I need to update the main .bib file to be a synced copy of our group library, and then ensure all citations below are in that library (and that the keys match.) -->
...
n_all <- nrow(df_all)
df <- df_all %>% 
  filter(exclude %in% "include") # only those who met the inclusion criterion
n <- nrow(df)
...
# but need to summarise exclusion criteria here]
  # exclude those who are younger than 18
  # only those who say "1" to eligibility are included; all other responses are excluded
  # those whose `status` variable is an 'IP Address' are included

age <- df_all %>%
  dplyr::count(age_lab) 

el <- df_all %>%
  dplyr::count(eligibility_lab) 

status <- df_all %>% 
  dplyr::count(status_lab)
...
# gender --> need to run through gendercoder
# high school status
# high school psyc 
...
18 years (*n* = `r age[1,2]`). We also screened out individuals who had already 
...
<<<<<<< HEAD
question (*n* = `r el[6,2]`). We also screened out an additional  participants
=======
question (*n* = `r el[6,2]`). We also screened out an additional XX participants
>>>>>>> f87ca90fd15c03630fa16bcdd0353e907be14f55
who met our eligibility criteria, but did not respond to any of the main items
in the study. 

The remaining `r n` participants
were eligible based on starting their first unit of study in psychology at 
...
18-24 years old (*n* = `r age[2,2]`), `r age [3,2]` 
reported that they were 25-34 years old, `r age [4,2]` were 35-44 years old,
`r age [5,2]` were 45-54 years old, `r age [6,2]` were 55-64 years old, and
`r age [7,2]` were 65 years or older. 
