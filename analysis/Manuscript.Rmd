...
The abstract also doesn't currently knit into the final document for some reason?
...
#metadata <- here::here("survey", "data", "students_metadata.csv") %>% 
 ...
 #dplyr::select (-c("...10", "note:"))

#MW: I have commented this chunk out out temporarily because it might need tweaking - it assumes a subfolder in "survey" called "data"
#and the last select line doesn't seem to run (no column with that name recognised)
```
...
<!--I feel like citing tweets is going to cause arguments with a reviewer, but not sure what else to do here.-->
...
<!-- I wonder about saying something somewhere about how the items we developed (and openly shared) in this study could be useful for teachers who might wish to assess their own students' beliefs about open science practices in psychology?
Also, if anyone else has any good citations on the value of exploratory research, can you let me know?
-->
...
<!-- It's common to put participants first, but might the description flow more smoothly if we put that short Procedure subsection first? That way they know how we recruited people before we get to thinks like exclusions and screening -->
...
`r age [4,2]` were 45-54 years old, and `r age [5,2]` was 55-64 years old. The majority of participants (*n* = `r gender[1,2]`) reported that they were female, 
...
<!--
MP: Seems like we are missing some nationalities here – US nationalities? NZ? Or is this just the top 3?
-->
...
MW: The table captions/crossreferences may not be working - see the compiled document.
-->
...
<!--MW: We need to add in the actual numbers of collaborators/institutions above - we should have a record somewhere in our Google Drive-->

## Data analysis
<!--MW: We had this subheading in here but might not need it? The analysis is pretty clear from the descripion below-->
...
Despite efforts to popularize the replicability of research the past decade, and despite popular media attention, most incoming psychology students in our sample were unaware of the replication crisis. Encouragingly, on the other hand, most tended to endorse open science norms more-so than counternorms. Notably this wasn't the case for all open science practices (namely sharing of pre-prints and avoiding HARKing). It may be interesting to consider whether student norms regarding the sharing of pre-prints would be expected to have shifted since this data was collected given the rapid increase in the dissemination of pre-prints related to COVID-19. On the other hand, it may well be that scientific training has enshrined the practices of peer review and generating hypotheses after analyses (e.g., Bem, 2004) as essential scientific practices that are robust to change.
...
<!-- MW: Should that last bit say registered reports?
There's also quite a bit of manually written numeric info here - we should probably convert that to R code?
-->
...
<!-- Since the journal asks for a Teaching Implications subsection in the structured abstract, I'm thinking it'd make sense to have a Teaching Implications subsection in the discussion, about here? -->
...
Our questions focused primarily on attitudes and beliefs so that we could better document the implicit understanding that guided students’ thoughts about the research process. However, we did not probe students' factual knowledge of methodological findings or concepts. It’s quite possible that reform in secondary school instruction has improved students’ understandings of more fundamental statistical (e.g., confidence interval estimation) and design (e.g., direct replications) open science considerations without necessarily impacting their attitudes and beliefs. Yet, research at the tertiary level casts doubt that the disciplinary reform from the past 2 decades has had much impact on the content of our teaching [@friedrichReplicatingNationalSurvey2018].
...
Figure XX displays the histograms for each of the norms in practice items. 

```{r norms in practice histogram}
...
#[breadcrumb: need to update the theme & the practice names]
