...
library(tidyselect) 
library(psychometric) # for correlation confidence intervals
...
The open science practices assessed in our survey included: critical thinking, preregistration, registered reports, p-hacking, hypothesising after the results are known (HARKing), providing sufficent information of replicability, preprints, open materials, open data, and open access publishing. Each practice was elucidated by a pair of items (pro-open science; counter-open science) in the context of a common scenario, "Imagine that Deborah is a psychology researcher who has designed a study to test a specific hypothesis." There were 20 scenarios in total (see Table X). Participants rated their agreement with each prescriptive statement on a 1 (strongly disagree) to 5 (strongly agree) scale.
...
In general, the students seemed to endorse open science practices: For seven of the ten open science practices, agreement with the norm item was stronger than for the corresponding counternorm. However, the extent to which participants endorsed open science norms varied very substantially across different practices. Participants tended to agree or strongly agree that p-hacking should be avoided, and that researchers should provide sufficient information about studies to permit replicability. On the other hand, they were very skeptical of the practice of sharing preprints prior to peer review, and to a lesser extent about sharing open data. There also a degree of equivocation about registered reports, with very similar levels of agreement with both norm and counternorm for this item. This may be relatively unsurprising, given that the registered reports are perhaps the most recent innovation in the list of practices.
...
```{r, difference between norm and counternorm}
 ...
 # \@ref(tab:chunk-name)  [JB removed this from the text above to see if that solves
  # our table caption issue]
...
prac_counts <- norm_cnorm %>% 
    dplyr::group_by(practice) %>%  
    count()

# calculate means
prac_ms_sds <- norm_cnorm %>% 
    dplyr::group_by(practice) %>%  
...
practice_means <- prac_means %>% 
  select(practice,Norm, Counternorm, Difference)
...
# I'm sure there's a way to automate it, but I can't figure it out...just hard code it
# all_cnorms <- df %>% 
#     select(ends_with("_cnorm"))
#   
# corr <- round(cor(all_), ends_with("_cnorm"),2)))
# 
# 
#   all_norms %>% 
#     cor(ends_with("_norm"), ends_with("_cnorm"))
#   
#  
#   cor(all_norms, method = "pearson") %>% 
#   as.tibble()
# 
# all_correlation <- df %>% 
#     select(c(ends_with("_norm"), ends_with("_cnorm"))) %>% 
#   mutate(item = row.names(.)) %>% 
#   select(item, everything())

# this code works, but I must be able to do this all at once...

# breadcrumbs: when I come back to this, I need to combine these in a tibble
  # and then join it with the existing table...I probably need to mutate, but I 
  # need to sort out how to do this...

# practice_corr <- practice_means %>% 
  # mutate() 
...
## Norms in practice
Participants were also asked to estimate the proportion of psychology studies estimated globally which apply ten practices (corresponding to the ten norm-counternorm pairs) . For many of these practices, participants estimates were near the midpoint of 50%. This said, participants perceived some practices as being applied relatively infrequently: For example, the mean esitmate of the prevalence of HARKing, preprint sharing before publication, and open data sharing were all near 30%. Participants seemed to perceived that the most common practice was providing enough information in written reports to permit replication without asking any questions of the original researchers. It is perhaps fair to say that participants mean estimate that `r paste(round(mean(df$detailed_methodology, na.rm = TRUE), 2), "%", sep = "")` of studies have this characteristic represents a rather optimistic projection.
...
Interestingly, the vast majority of participants had not heard of "the replication crisis". Participants' estimates of the replicability of psychological studies was also near the midpoint of the possible range (ABOUT 56%). Interestingly, this estimate is fairly consistent (albeit perhaps slightly more optimistic) than empirical estimates from large-scale studies. However, it could plausibly simply reflect a great deal of uncertainty on the part of participants.
