...
```{r, difference between norm & counternorm, eval = FALSE}

# select all columns that relate to norms & counternorms
diff <- df %>% select(c(id, ends_with("norm")))
...
# select columns that relate to norms
norms <- df %>% select(c(id, ends_with("_norm")))

# select columns that relate to counternorms
cnorms <- df %>% select(c(id, ends_with("_cnorm")))




# breadcrumbs - pivot long

norm_long <- norms %>% 
  pivot_longer (cols = ends_with("norm"),
                names_to = "practice",
                values_to = "rating") 
  str_remove(norm_long, "_norm")

norm_wide <- diff %>% 
  pivot_wider(names_from = ends_with("_norm"),
              values_from = c(ends_with("_norm"), ends_with("_cnorm"))
)
```
