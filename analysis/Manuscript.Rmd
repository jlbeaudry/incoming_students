...
# import the metadata
#MW: I've removed the argument lazy = FALSE from the read_csv command which was causing an error for me - I don't think it's an argument used by that function?
metadata <- here::here("data", "students_metadata.csv") %>%
read_csv(col_names = TRUE, skip_empty_rows = TRUE) %>%
filter(!is.na(old_variable))

# relabel the variable names [completed in preprocessing]
# df <- meta_rename(df, metadata = metadata, old = old_variable, new = new_variable)
...
MW: Cool - I've put that in the conclusion of the discussion along with a link. Hope that looks ok. I don't think we can quite fit it in the abstract with the 200 word limit.
-->
...
For teachers interested in systematically surveying their students' existing understanding of open science practices in psychology, our survey questions could be a useful resource. These questions are available along with our data and analysis code at https://osf.io/fz6ya/?view_only=46705e978a964cf6b872f4203d718fbe 

<!---
MW: We still need to upload the data and code there before submitting, but just popping this placeholder in for now.
I guess we'll need to leave out a couple of the files that have identifiable data in them?
I've also uploaded an exported copy of the survey on the OSF project, with author details blinded.
-->
...
practice_labels <- c(
  ...
  detailed_methodology = "Information\nfor replication",
  harking = "HARKing",
